...
def make_players_table(data, cur, conn):
    cur.execute("CREATE TABLE IF NOT EXISTS Players (id INTEGER PRIMARY KEY, name TEXT, position_id INTEGER, birthyear NUMERIC, nationality TEXT)")
    for value in data['squad']:
        id = value['id']
        name = value['name']
        cur.execute("SELECT id FROM Positions WHERE Positions.position = ?", (value['position'],)) #DOUBLE CHECK PLURALS
        position_id = cur.fetchone()[0]
        birthyear = int(value['dateOfBirth'][:4]) #FIX HERE TO ONLY HAVE YEAR- DOB formate is YYYY-MM-DD
...
def birthyear_nationality_search(age, country, cur, conn):
    list = []
    year = 2023 - age
    cur.execute("SELECT name, birthyear FROM Players WHERE Players.nationality = ? AND birthyear < ?", (country, year,))
    for row in cur.fetchall(): 
        list.append((row[0], country, row[1])) #... in my dreams 
    conn.commit()
    return list
